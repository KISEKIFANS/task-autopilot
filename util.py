...
if not os.path.exists(Conf.log_path):
    ...
    print(f'检测到日志文件夹{Conf.log_path}不存在，创建完成')
...
def init(new_game):
    ...
    print_and_log('初始化开始，当前目录' + os.getcwd())
    if not os.path.exists(Conf.screenshot_path):
        os.makedirs(Conf.screenshot_path, exist_ok=True)
        print_and_log(f'检测到截图文件夹{Conf.screenshot_path}不存在，创建完成')
...
def find_img(seq, img_target, timeout, mission):
    if img_target == "MATCH_YES":
        ...
        count = 0
        while count < timeout:
            center, score = get_img_center(seq, img_target)
            if score > Conf.score_threshold:
                print_and_log(f'任务{seq}正在进行第{count + 1}次匹配，score={score}，未检测到有效目标')
                count += 1
                time.sleep(1)
            else:
                print_and_log(f'任务{seq}正在进行第{count + 1}次匹配，score={score}，检测到有效目标，{mission}')
                return center
        print_and_log(f'任务{seq}超过重试次数')
...
def process_mission(seq_start):
    ...
    for line in lines:
        ...
        if seq_is_found:
            ...
            print_and_log(f'seq={seq},img_target={img_target},action_y={action_img_found},action_n={action_img_not_found},timeout={timeout},mission={mission}')
                ...
                # 当有click关键字
                    ...
                    if action_img_found.find('@') == -1:
                        ...
                        if find_img_result == 'DIRECT_YES':
                            print_and_log('MATCH_YES时不能直接click')
                        else:
                            mouse_click(find_img_result)
                    ...
                    # 当带有@，点相对坐标或绝对坐标
                        ...
                        if not abs_x.startswith('*') and not abs_y.startswith('*') and find_img_result == 'DIRECT_YES':
                            print_and_log('MATCH_YES时click不能跟相对座标')
                        ...
                        # 非直接成功事件或绝对坐标：
                            ...
                            mouse_click(find_img_result)
                            print_and_log(f'{find_img_result}')
                    ...
                    mouse_slip_around(find_img_result, dis_x, dis_y)
                else:
